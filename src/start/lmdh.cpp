...
public:
    void decompose(int max_tree_width, ofstream& output, int start)
    {
        int tree_width = 1;
        ...
        for (bool is_ret : visited)
            if (is_ret)
                ret_cnt++;
        export_info_step(output, max_tree_width, remove_cnt, true_num_nodes, int(duration.count()), ret_cnt);
    }
};
...
int main(int argc, char* argv[])
{
    ...
    type = "lmdh";
    string output_name = "output/" + type + "-" + to_string(width) + "-" + filename + ".output";
    ...
    for (int i = 0; i < TRIALS; ++i) {
        ...
        int start = g.random_node();
        g.decompose(width, output, start);
    }
