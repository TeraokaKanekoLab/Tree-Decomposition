...
int main(int argc, char* argv[])
{
    ...
    for (int per = 1; per <= SAMPLE; ++per) {
        for (; index < t.num_bags() * per / SAMPLE; ++index) {
            ...
            if ((double)per / SAMPLE * 100 > 20.0)
                cout << (double)per / SAMPLE * 100 << " ISS: " << t.subtree_size_induced_by(bcs[index].second) << " child: " << t.num_children(bcs[index].second) << endl;
        }
        if ((double)per / SAMPLE * 100 > 20.2)
            break;
        ...
        // cout << (double)per / SAMPLE * 100 << " " << (double)cnt1 / top1.size() * 100 << " " << (double)cnt5 / top5.size() * 100 << " " << (double)cnt10 / top10.size() * 100 << " " << (double)cnt15 / top15.size() * 100 << " " << (double)cnt20 / top20.size() * 100 << " " << (double)cnt25 / top25.size() * 100 << " " << endl;
        // if (bcs[index].first <= 0)
        //     break;
    }
