...
int main(int argc, char* argv[])
{
    ...
    string output_name = "output/bc-dist_root/" + width + "-" + filename + ".output";
    ...
    for (auto bc_pair : bcs) {
        ...
        // int dl = g.degree(nd);
        output << dl << " " << bc << " " << g.degree(nd) << " " << t.subtree_size_induced_by(nd) << " " << g.compute_clustering_coefficient(nd) << endl;
    }
